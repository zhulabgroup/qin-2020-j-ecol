...
plot_lme_coefs_richness <- function(coefs_table, levels_display, diversity_metric) {
  ...
  coefs_table %>%
    ...
    scale_colour_manual(
      labels = c("Main", "GC interactions", "Plant-soil"),
      values = cbbPalette
    ) +
    guides(fill = FALSE) +
    xlab("") +
...
levels_order_threemodels <- c(
  "n2", "co22", "heat2", "precip2", "wildfire.20032", "burn.20112",
...
coefs_main %>%
  ...
  mutate(
    treatment = factor(treatment, levels = levels_order_threemodels),
