...
varimp_gdm_selection <- function(physeq, preds, outfile_name = "gdm_outfile",
  ...
  if (include_plant_dissim) {
    sitepair.sel <- formatsitepair(
      bioData = bioData, bioFormat = 1, dist = "bray",
  ...
  if (!selection) {
    ...
    if (test_var_imp) {
      ...
      if (include_geo) {
        gdm.selection0 <- gdm.varImp(sitepair.sel,
          parallel = TRUE, geo = TRUE,
          outFile = paste0(outfile_name, "_parsim"), nPerm = nperm,
          fullModelOnly = TRUE
        )
      } else {
        gdm.selection0 <- gdm.varImp(sitepair.sel,
          parallel = TRUE, geo = FALSE,
          outFile = paste0(outfile_name, "_parsim"), nPerm = nperm,
          fullModelOnly = TRUE
        )
      }
      ...
      print(paste0(
        "Saved parsimonious model's summary, variable importance, and variable significance to ",
    ...
    if (include_geo) {
      gdm.selection <- gdm.varImp(sitepair.sel,
        parallel = TRUE, geo = TRUE,
    ...
    print(paste0(
      "Saved model selection results to ",
    ...
    if (next_test_parsim_model) {
      if (include_geo_parsim & include_plant_dissim_parsim) {
        return(varimp_gdm_selection(physeq, preds_parsim, outfile_name,
          include_geo = TRUE,
      ...
      if (include_plant_dissim_parsim) {
        sitepair.parsim <- formatsitepair(
          bioData = bioData, bioFormat = 1, dist = "bray",
...
preds_interact <- c(
  "id", "lon", "lat", "n", "co2", "heat", "precip", "burn.2011", "wildfire.2003",
...
gdm.ITS.plantsoil <- varimp_gdm_selection(ITS.rare,
  preds = gdm.ITS.plantsoil.preds,
  include_geo = TRUE, include_plant_dissim = TRUE,
  selection = FALSE, test_var_imp = FALSE
)

gdm.BAC.main.preds <- c("co2", "precip", "wildfire.2003", "n", "heat", "burn.2011")
gdm.BAC.main <- varimp_gdm_selection(BAC.rare,
  preds = gdm.BAC.main.preds,
  include_geo = TRUE, include_plant_dissim = FALSE,
  selection = FALSE, test_var_imp = FALSE
)

gdm.BAC.interact.preds <- c("cp", "wildfire.2003")
gdm.BAC.interact <- varimp_gdm_selection(BAC.rare,
  preds = gdm.BAC.interact.preds,
  include_geo = TRUE, include_plant_dissim = FALSE,
  selection = FALSE, test_var_imp = FALSE
)

gdm.BAC.plantsoil.preds <- c("co2", "wildfire.2003")
gdm.BAC.plantsoil <- varimp_gdm_selection(BAC.rare,
  preds = gdm.BAC.plantsoil.preds,
