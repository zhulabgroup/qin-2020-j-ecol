...
plant.rich.all <- apply(plants.prune, 1, function(x) {
  sum(x != 0 & !grepl("UN", names(plants.prune)))
})
...
sampleData %>%
  dplyr::rename(
    lon = x, # These are custom-scaled coordinates based on aerial imagery
    lat = y
  ) ->
sampleData
...
merge(rich.ITS, rich.BAC, by = "id", all = TRUE) %>%
  dplyr::rename(
    ITS.observed = observed.x, ITS.shannon = shannon.x,
  ...
  dplyr::select(
    id, n, co2, heat, precip, burn.2011, wildfire.2003, perc.n, c.n, water.content, ph, npp.std, plant.rich, plant.shannon, ITS.observed, ITS.shannon, BAC.observed, BAC.shannon, NMDS.Plants.1, NMDS.Plants.2, plt,
    contains("NMDS")
  ) %>% # -> rich.microbial
